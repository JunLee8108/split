...
- [x] 템플릿 라이브러리 — 내장 템플릿 7개(PlanTemplate + 설명), + 메뉴(직접 만들기 / 템플릿에서 고르기), 템플릿 시트에서 내 플랜으로 복사. 첫 실행 자동 삽입은 400m × 12, 3분/1분 × 6 두 개만
- [x] 구간 목표 — 거리 구간은 목표 시간, 시간 구간은 목표 거리(goalValue). 편집 시트에 목표 토글·휠·예상 페이스(km/mi)·페이스 표 시트(행 탭으로 선택). 세션 화면 목표 줄(남은/초과), 구간 종료 음성 결과, 랩 행 목표 차이, 요약·상세 목표 달성 n/m
- [x] 세션 화면 정보 위계 — 1층 남은 거리/시간(92pt), 2층 목표까지/초과 또는 구간 경과(52pt, 구간 색), 3층 이번 구간 거리·시간, 하단 페이스·총 거리·총 시간. 단위는 숫자에 붙이고 라벨은 숫자 아래로 통일
- [x] 기록 화면에서 이번 주 요약 카드·차트 제거. 월별 카드 목록만
